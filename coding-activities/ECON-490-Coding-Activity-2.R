...
# The basic OLS regression function in R is lm(). To regress hhincome on
# renter and employed, we can run the following:
...
acs.data$predicted.hhincome <- ols.model$fitted.values
...
prediction.30 <- 
  q6.model$coefficients["(Intercept)"] + 
  q6.model$coefficients["age"]*30 + 
  q6.model$coefficients["employed"]
